...
String Unit = "C3";                                     // *** MUST CHANGE THIS TO YOUR ASSIGNED UNIT (A1, A2, ..., A5; B1, B2, ..., B5; C1, C2, ..., C5; D1, D2, ..., D5)
String I2CSensorBeingUsed = "VEML6070";                   // *** MUST CHANGE THIS TO YOUR ASSIGNED I2C SENSOR ("VEML6070", "VEML7700", "AS7262", "SI1145")
String AnalogSensorBeingUsed = "ALS PT-19";            // *** MUST CHANGE THIS TO YOUR ASSIGNED ANALOG SENSOR ("GUVA-S12SD", "ALS-PT19")
...
void getHeader(String I2CSensor, String AnalogSensor) {
  ...
  xBeeHeader = String(Unit) + spacer + "AccelX, AccelY, AccelZ, MagX, MagY, MagZ, TempF, TempC, Photo, " + AnalogSensor + spacer;
...
void updateDataStrings(String I2CSensor) {
  ...
  xBeeData = xBeeData = Unit + spacer + String(accelerometer[0]) + spacer + String(accelerometer[1]) + spacer + String(accelerometer[2]) + spacer + String(magnetometer[0]) + spacer + String(magnetometer[1]) + spacer + String(magnetometer[2]) + spacer + String(currentTempF) + spacer + String(currentTempC) + spacer + String(PhotoresistorData) + spacer + String(AnalogSensorData) + spacer;
